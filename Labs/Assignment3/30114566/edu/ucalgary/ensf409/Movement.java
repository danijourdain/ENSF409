...
public class Movement implements Cloneable, FormattedOutput {
        ...
        if(movementMatch.find()) {
            ...
            for(Directions direction: Directions.values()) {
                if(movementMatch.group(2).equals(direction.name())) {
                    this.direction = movementMatch.group(2).toString();
                }
            }

            if(this.action == null || this.direction == null) {
                throw new IllegalArgumentException("Action and/or Direction are invalid values");
            }
        }
        else {
            throw new IllegalArgumentException("Action and/or Direction are in an invalid form");
        }
    }
    ...
    public String getFormatted() {
        ...
        output.append("Action: " + this.getAction());
